...
### 数据库
数据库使用 SQLite，启动时通过 `Base.metadata.create_all()` 自动建表（见 `src/app/db/base.py`）。新增字段通过 `init_db()` 中的迁移函数自动追加（`ALTER TABLE ADD COLUMN`），无需手动操作。新增模型字段时需在 `src/app/db/__init__.py` 添加对应的 `_migrate_xxx` 函数。`alembic` 在依赖中但项目未使用独立迁移目录。
...
- **模拟器 I/O 池**（`get_emulator_io_pool()` / `run_in_emulator_io()`）：每个模拟器一个 `max_workers=1` 的单线程池，序列化该模拟器的 ADB 命令防止并发冲突
...
- `SystemConfig`（单行配置表）：存储全局系统设置（启动方式、ADB 路径、全局任务开关、默认失败延迟等），通过 `/api/system/config` 动态修改
...
|--------|----------|------|
| 100 | INIT | 起号 |
| 99 | INIT_COLLECT_REWARD | 起号_领取奖励 |
| 98 | INIT_RENT_SHIKIGAMI | 起号_租借式神 |
| 96 | INIT_SHIKIGAMI_TRAIN | 起号_式神养成 |
| 95 | INIT_EXP_DUNGEON | 起号_经验副本 |
| 94 | INIT_FANHE_UPGRADE | 起号_升级饭盒 |
| 93 | INIT_NEWBIE_QUEST | 起号_新手任务 |
| 92 | INIT_COLLECT_JINNANG | 起号_领取锦囊 |
| 91 | COLLECT_ACHIEVEMENT | 领取成就奖励 |
| 90 | ADD_FRIEND | 加好友 |
| 80 | COOP | 勾协 |
| 70 | XUANSHANG | 悬赏 |
| 65 | DELEGATE_HELP | 弥助 |
...
| 39 | WEEKLY_SHARE | 每周分享 |
| 38 | DAILY_SUMMON | 每日一抽 |
| 37 | SUMMON_GIFT | 召唤礼包 |
| 36 | DOUJI | 斗技 |
| 35 | CLIMB_TOWER | 爬塔 |
| 34 | DUIYI_JINGCAI | 对弈竞猜 |
| 21 | EXPLORE | 探索突破 |
| 20 | REST | 休息 |
...
### 调度链路
- 调度链路唯一实现为 `Feeder + ExecutorService`，legacy 调度器（simple_scheduler）已完全移除
...
### 环境配置
- 首次开发复制 `.env.example` 为 `.env`，完整配置项列表见 `.env.example`，以下仅列出关键项：
  - `DATABASE_URL`：SQLite 路径（默认 `sqlite:///./data.db`）
  - `MUMU_MANAGER_PATH`、`ADB_PATH`：模拟器和 ADB 路径
  - `OCR_MODEL_DIR`：PaddleOCR 模型目录（默认 `<项目根>/ocr_model/`）
  - `JWT_SECRET`：首次启动自动生成并追加到 `.env`
  - `RUN_MODE`：运行模式（默认 `local`，可选 `cloud`，云端模式需配置 `CLOUD_*` 系列参数）
