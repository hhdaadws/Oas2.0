...
```
前端开发端口 9000（`vite.config.js`），后端端口 9001。Vite 已配置 `/api` → `http://127.0.0.1:9001` 和 `/ws` → `ws://127.0.0.1:9001` 代理。
...
### 数据库
SQLite，启动时通过 `Base.metadata.create_all()` 自动建表（`src/app/db/base.py`）。新增字段通过 `init_db()` 中的 `_migrate_xxx` 函数自动 `ALTER TABLE ADD COLUMN`。新增模型字段时需在 `src/app/db/__init__.py` 添加对应迁移函数。
...
### 提交规范
`feat(scope): ...`、`fix: ...`、`chore: ...`

### 打包发布
```bash
python build.py   # 一键打包：前端构建 → PyInstaller → 后处理
```
输出到 `dist/YYSAutomation/`。打包规范文件为 `yys_automation.spec`，PyInstaller 钩子在 `hooks/` 目录。
...
3. **WorkerActor**（`src/app/modules/executor/worker.py`）：每个模拟器绑定一个 Worker。通过 `inbox` 队列接收批次，串行执行其中的每个 `TaskIntent`。同批次任务共享 `EmulatorAdapter` 和 `UIManager` 实例，避免重复启动游戏。Worker 通过 EmulatorAdapter 心跳检测任务卡死（180 秒无 I/O 活动则取消任务）。

**云端模式**（`src/app/modules/cloud/`）：当 `RUN_MODE=cloud` 时，`CloudTaskPoller` 替代 Feeder 从远程管理后台拉取任务并注入 ExecutorService。运行模式通过 `RuntimeModeState` 全局切换。相关配置项前缀为 `CLOUD_*`。
...
- 弥助任务使用固定时间点触发：`00:00/06:00/12:00/18:00`（硬编码于 `worker.py` 的 `_MIZHU_FIXED_TIMES`）
- **新增任务类型的完整指南见 `docs/add-new-task.md`**
...
- **UIDetector**（`src/app/modules/ui/detector.py`）：截图后通过模板匹配识别当前界面（返回 `UIDetectResult`）
- **UIGraph**（`src/app/modules/ui/graph.py`）：UI 界面有向图，BFS 规划从当前界面到目标界面的导航路径。每条边（`Edge`）包含 `Action` 列表（tap/swipe/sleep/tap_anchor）
- **UIManager**（`src/app/modules/ui/manager.py`）：对外统一入口。`ensure_game_ready()` 是任务执行前的标准调用。`ensure_ui(target)` 自动规划路径并逐步导航
...
- **专项视觉检测器**（`src/app/modules/vision/`）：`explore_detect.py`、`tupo_detect.py`、`yuhun_detect.py`、`battle_lineup_detect.py`、`color_detect.py`
...
`src/app/core/thread_pool.py` 提供三种全局线程池：

- **I/O 池**（`run_in_io()` / `run_in_db()`）：ADB subprocess、同步 DB 操作。大小 `max(8, emulator_count * 2 + 4)`，上限 32
- **计算池**（`run_in_compute()`）：OpenCV 模板匹配、OCR。大小 `max(4, cpu_count // 2)`，上限 24
- **模拟器 I/O 池**（`run_in_emulator_io()`）：每个模拟器一个 `max_workers=1` 单线程池，序列化 ADB 命令防止并发冲突
- 可通过 `.env` 的 `IO_THREAD_POOL_SIZE` / `COMPUTE_THREAD_POOL_SIZE` 手动覆盖

### 数据模型

- `Email` 1→N `GameAccount`（一个邮箱最多 4 个游戏账号，不同区服）
- `GameAccount.task_config`（JSON 字段）：存储每种任务的 `enabled`、`next_time` 等配置
...
- `CoopAccount`：勾协账号库，通过 `CoopPool` 与 GameAccount 配对
- `SystemConfig`（单行配置表）：全局系统设置，通过 `/api/system/config` 动态修改

### 任务类型与优先级

定义在 `src/app/core/constants.py`，中文 Enum 值。`TASK_PRIORITY` 字典控制执行顺序（数值越大越优先）。完整列表见 `AGENTS.md`。
...
**实时通信**：通过 `python-socketio` WebSocket 推送实时状态更新。
...
### 调度链路
- 调度链路唯一实现为 `Feeder + ExecutorService`
- `/api/tasks/scheduler/start|stop|status` 控制 Feeder + ExecutorService 的启停

### 仪表盘数据源
- `/api/dashboard` 使用 `executor_service.queue_info()` / `running_info()` 作为数据源

### 任务配置更新
- `task_config` 更新必须采用"默认配置 + 现有配置 + 本次提交"的合并策略，禁止用默认值覆盖
- 使用 `model_dump(exclude_unset=True)` 获取请求体有效字段
- 任务启用判定：仅 `enabled is True` 视为启用
...
### 环境配置
- 首次开发复制 `.env.example` 为 `.env`，完整配置项列表见 `.env.example`
- `JWT_SECRET`：首次启动自动生成并追加到 `.env`
- `RUN_MODE`：运行模式（默认 `local`，可选 `cloud`）
...
### 阵容系统
- `src/app/modules/lineup/` 管理战斗阵容配置
- `LINEUP_SUPPORTED_TASKS` 定义支持阵容的任务列表
- 执行器通过 `get_lineup_for_task(account.lineup_config, "任务名")` 获取分组和位置

### 桌面端
- 打包模式（`launcher.py`）：PyInstaller 入口，后台线程启动 uvicorn + pywebview 桌面窗口（1280×860）
- 开发模式：Electron 壳层（`desktop/main.js`），包装前端 Web 页面为桌面应用

### Windows 注意事项
- PowerShell 读取文件时若出现乱码，使用 `-Encoding UTF8` 参数
